...
const Contact = () => {
  ...
  const handleSubmit = (e) => {
    ...
    emailjs.send(
      ...
      console.log(error)
      showAlert({
        show: true, text: 'Something went wrong!', type: 'error'
      })
    })
  }
  return (
    <section className="relative flex lg:flex-row flex-col max-container ">
      ...
      <div className="flex-1 min-w-[50%] flex flex-col">
        ...
        <form className="w-full flex flex-col gap-7 mt-14" onSubmit={handleSubmit} >
          ...
          <label className="text-black-500 font-semibold">
            Your Message
            <textarea name="message" rows={5} className="textarea" placeholder="Let me know I can help you!" required value={form.message} onChange={handleChange} onFocus={handleFocus} onBlur={handleBlur} />
          </label>
          <button type="submit" className="btn" onBlur={handleBlur} onFocus={handleFocus} disabled={isLoading}>
            {isLoading ? 'Sending...' : 'Send Message'}
          </button>
        </form>
      </div>
      <div className="lg:w-1/2 w-full lg:h-auto md:h-[550px] h-[350px]">
        <Canvas className="" camera={{ position: [0, 0, 5], fov: 75, near: 0.1, far: 1000 }} >
